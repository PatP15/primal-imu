...
@hydra.main(version_base=None, config_path="../primal/configs", 
            config_name="train_diffusion_acceleration")
def train(cfg: DictConfig) -> None:
    """Main training function for acceleration-based motion diffusion models.
    ...
    This script is identical to train.py except it uses the acceleration-based
    ARDiffusionAcceleration model instead of ARDiffusion.
    
    Args:
        cfg (DictConfig): Hydra configuration object containing:
            - data: Dataset configuration (AMASS, batch size, etc.)
            - model: Model architecture and hyperparameters (ARDiffusionAcceleration)
