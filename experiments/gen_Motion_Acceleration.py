...
def predict_mop(args) -> None:
    ...
    outputdir = os.path.join('outputs/MotionRealismAcceleration', f'{os.path.basename(args.expdir)}')
    os.makedirs(outputdir, exist_ok=True)
    outputfile = os.path.join(
        outputdir,
        f'{args.dataset}_ema-{args.use_ema}_reproj-{args.use_reproj_kpts}_inertial-{args.use_inertialization}.pkl')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--frame_skip', type=int, default=120,
    ...
    model = ARDiffusionAcceleration.load_from_checkpoint(
        ckptpath, 
        map_location='cuda:0',
        strict=False
    )
    ...
    ## define the traj. This is actually not used but to keep the code consistency.
    ori_angle_deg = np.deg2rad(90)
    ori = [np.cos(ori_angle_deg), 0, np.sin(ori_angle_deg)]
    ori = torch.tensor(ori)[None,None,...] #[b=1,t=1,3]
    ori = ori.float().to(model.device)

    ## other params
    # nt_tw = model.hparams.data.seq_len-2 # Corrected for acceleration model
